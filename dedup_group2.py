...
file_handler = logging.FileHandler('logs_2_2.txt')
    ...
    def run():
        ...
        for group_name in ["group_2_2"]:
            ...
            for name in data[group_name]:
                ...
                if group_name == "UbuntuIRC_ver2":
                    ds = ds
                if name == "CodePileReddit2019_ver2":
                    # convert meta column to string
                    ds = ds.map(lambda x: {"meta": json.dumps(x["meta"])}, num_proc=128)
            #    ds = ds.select(range(1000000)) 79%
            ...
            for i in range(len(group)):
                group[i] = group[i].remove_columns(["id"])
